...
import obj.inhirtance;
public class E07_hashMapEx {

	public static void main(String[] args) {
		// Map
		// key / value 가 이중으로 된 자료
		...
		map.put("ABC", 123);
		map.put("FF", 123);
		map.put("홍길동", 88); //이미 존재하는 키에 넣은 경우 V값을 덮어씌움
		//get(k) - 키를 넣으면 해당값 반환
		System.out.println(map.get("홍길동"));
		System.out.println(map.get("임꺽정"));
		
		//getOrDefault() - 키를 넣으면 해당값 반환, 값이 없으면 두번째 매개변수인 default값을 반환
		System.out.println(map.getOrDefault("ABC", 1000));
		System.out.println(map.getOrDefault("ABCD", 1000));
		//keySet() - Map의 모든 키들을 Set으로 반환
		for(String key : map.keySet()) {		//Set<String> s == map.leySet()
			System.out.printf("%s : %d \n" , key,map.get(key));
		}
		//entrySet() map의 entrySet을 반환
